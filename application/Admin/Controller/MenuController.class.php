...
class MenuController extends AdminbaseController {
    	...
    	if (IS_POST) {
    		...
    		if ($this->menu_model->create()) {
    			...
    			if ($this->menu_model->add()!==false) {
					...
					if($type==1){
						if($parentid>0){
							$parent_info=$this->menu_model->where(array('id'=>$parentid))->find();
							if($parent_info){
								$pname=strtolower("$parent_info[app]/$parent_info[model]/$parent_info[action]");
								$parentid=(int)M('AuthRule')->where(array('name'=>$pname,'type'=>1))->getField('id');
							}else{
								$parentid=0;
							}

						}
						$auth_rule_count=M('AuthRule')->where(array('type'=>1,'name'=>$name))->count();
						if(!$auth_rule_count){
							M('AuthRule')->add(array('name'=>$name,'type'=>1,'title'=>$menu_name,'parentid'=>$parentid));
						}
					}
    ...
    public function delete() {
        ...
        $menu = $this->menu_model->where(array("id" => $id))->find();
        $name=strtolower($menu['app']."/".$menu['model']."/".$menu['action']);

        M('AuthRule')->where(array('name'=>$name,'type'=>1))->delete();
    			...
    			if ($this->menu_model->save() !== false) {
					...
					if($type==1){
						...
						if(!$auth_rule_count){
							...
							M('AuthRule')->add(array('parentid'=>$parentid,'name'=>$name,'type'=>1,'title'=>$menu_name));
						}else{
							M('AuthRule')->where(array('type'=>1,'name'=>$name))->save(array('parentid'=>$parentid,"name"=>$name,"type"=>1,"title"=>$menu_name));
						}
					}
