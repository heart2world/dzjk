...
class UeditorController extends Controller {
	...
	public function uploadimg(){
		$upload_path=C('UPLOAD_ACTION_HOST').'&appKey='.C('UPLOAD_ACTION_KEY').'&module='.CONTROLLER_NAME.'&field='.ACTION_NAME;
		$ch = curl_init();
	...
	private function _ueditor_upload($config=array()){
		...
		if ($info) {
			...
			if(C('UPLOAD_TYPE')=='remote'){
				...
				$upload_path=C('UPLOAD_ACTION_HOST').'&appKey='.C('UPLOAD_ACTION_KEY').'&module='.CONTROLLER_NAME.'&field='.ACTION_NAME;
				$data = array(
					'upfile'=>'@'.$_SERVER['DOCUMENT_ROOT'].$url
				);
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL,$upload_path);
				curl_setopt($ch, CURLOPT_POST, 1);
				...
				if($result['state']=='SUCCESS'){
					@unlink ($_SERVER['DOCUMENT_ROOT'].$url);//删除服务器文件
					$result['url']=C('UPLOAD_REMOTE_URL').$result['url'];
				}
				...
				if(strpos($url, "https")===0 || strpos($url, "http")===0){
					...
					$image->thumb(1200, 1200)->save($_SERVER['DOCUMENT_ROOT'].$url);
