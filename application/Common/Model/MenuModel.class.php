...
class MenuModel extends CommonModel {
    ...
    public function admin_menu($parentid, $with_self = false) {
        ...
        if ($uid == 1) {
            //如果是超级管理员 直接通过
            $result = $this->where(array('parentid' => $parentid, 'status' => 1,'isshow'=>1))->order(array("listorder" => "ASC"))->select();
        ...
        foreach ($result as $v) {
            ...
            if (preg_match('/^public_/', $action)) {
                ...
                $count=M('AuthRule')->where(array('name'=>$rule_name,'type'=>1,'status'=>1))->count();
                if($count){
                    if (sp_auth_check_new($uid,$rule_name,1)){
                        $array[] = $v;
                    }
                }else{
                    $array[] = $v;
    ...
    public function menu_json2() {

        $data = $this->get_tree(0,3);
