...
class WechatAuth {
    ...
    //客服根路径
    private $kfapiURL = 'https://api.weixin.qq.com/customservice';
    ...
    public function menuCreate($button){
        ...
        $res['errcode']=0;
        $res['errmsg']='success';
        foreach($data['button'] as $key=>$vo){
                if($vo['type']=='click'){
                    $vo['key']=$vo['url'];
                    unset($vo['url']);
                }
                if(key_exists('sub_button',$vo)){
                    unset($vo['type']);
                    unset($vo['url']);
                    unset($vo['key']);
                }

            foreach($vo['sub_button'] as $k=>$v){
        ...
        foreach($data['button'] as $key=>$vo){
            if($vo['name']==''){
                ...
                $res['errmsg']='菜单名称不能为空';
                return $res;
            }elseif(mb_str_len($vo['name'])>20){
                $res['errcode']=1;
                $res['errmsg']=='菜单名称不能超过20个字';
                return $res;
            }
            ...
            if($vo['type']=='click'){
                if($vo['key']==''){
                    ...
                    $res['errmsg']='关键词不能为空';
                    return $res;
                }elseif(mb_str_len($vo['key'])>16){
                    $res['errcode']=1;
                    $res['errmsg']='关键词不能超过16个字';
                    return $res;
                }
            }elseif($vo['type']=='view'){
                if($vo['url']==''){
     ...
     * @return array          api返回结果
     */
    protected function kf_api($name, $data = '', $method = 'POST', $param = '', $json = true){
        $params = array('access_token' => $this->accessToken);
     ...
     * @param  string 客服昵称
     */
    public function kfCreate($kf_account,$nickname){
        $data = array('kf_account' =>$kf_account,'nickname' => $nickname);
        return $this->kf_api('kfaccount/add', $data);
    }

    /**
 * 获取客服信息
 */
    public function kfGetList(){
        $data = array();
        return $this->api('customservice/getkflist', $data);
    }
    ...
    public function inviteworker($kf_account,$invite_wx){
        $data = array('kf_account'=>$kf_account,'invite_wx'=>$invite_wx);
        return $this->kf_api('kfaccount/inviteworker', $data);
    }

    /**
     * 删除客服
     */
    public function delkf($kf_account){
        $data = array('kf_account'=>$kf_account);
        return $this->kf_api('kfaccount/del', null,'GET',$data);
    }

    /**
     * 获取客户会话状态
     */
    public function getOpenidSession($openid){
        $data = array('openid'=>$openid);
        return $this->kf_api('kfsession/getsession', null,'GET',$data);
    }

    //获取客服在线状态
    public function getonlinekflist(){
        return $this->api('customservice/getonlinekflist', null,'GET');
    }

    //发送客服消息
    public function sendMessage($openid,$content,$kf_account=''){
        $data['touser']=$openid;
        ...
        return $this->api('message/custom/send', $data,'POST');
    }
    ...
    public function kfsessionCreate($kf_account,$openid){
        $data = array('kf_account' =>$kf_account,'openid' => $openid);
        return $this->kf_api('kfsession/create', $data);
    }
}
