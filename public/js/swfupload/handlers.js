...
function att_show(serverData,file)
{
	...
	console.log(ext);
	console.log(444);
	var filenamestr =getfilename(filename);
	console.log(filenamestr);
	/*修改结束*/
	if(ext == 1) {
		if(filenamestr == '.pdf')
		{
			img = '<div class="get selected" id="aid-'+id+'"><a class="del" href="javascript:;">删除</a> <img onclick="att_cancel(this,'+id+',\'upload\')" width="87" height="98" src="/data/upload/pdf.png" data-id="'+id+'" data-path="'+src+'" alt="上传完成" title="'+filename+'"><input type="text" class="J_file_desc" name="flashatt['+id+'][desc]" placeholder="请输入描述" value="'+filename+'" style="width:68px"></div>';
		}else{
