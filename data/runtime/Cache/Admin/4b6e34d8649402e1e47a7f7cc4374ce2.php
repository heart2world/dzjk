...
<div class="wrap js-check-wrap" id="seckill">
    ...
    <form  class="form-horizontal"  method="post" @submit.prevent="submit_form">
        ...
        <div class="control-group" >
            ...
            <div class="controls">
                <select class="select_2 labellist" name="cate_id" style="width:100px">
                    <?php if($info): ?><option value="<?php echo ($label["id"]); ?>"><?php echo ($label["name"]); ?></option>
                        <?php else: ?>
                        <option value="0">请选择标签</option><?php endif; ?>
               ...
               <span style="margin-left: 10%;font-size: 14px;font-weight: normal;line-height: 20px;">
                       推荐到首页：
                        <input <?php if($info["tj"] == q): ?>checked<?php endif; ?> type="checkbox" id="checkbox-id" class="istjshouye"> 是
               </span>
            </div>
        </div>
        ...
        <div class="control-group" >
            ...
            <div class="controls">
                <select class="select_2 zzp" name="cate_id"  style="width:100px">
                    <?php if($info): ?><option value="<?php echo ($author["id"]); ?>"><?php echo ($author["nickname"]); ?></option>
                        <?php else: ?>
                        <option value="0">平台发布</option><?php endif; ?>
        ...
        <div class="control-group" >
            ...
            <div class="controls">
                <input type="text" value="<?php echo ($info["title"]); ?>" class="title" name="keywords" placeholder="30字以内" style="width: 200px;"  >&nbsp;&nbsp;
            </div>
        </div>
        <div class="control-group" >
            ...
            <div class="controls">
                <fieldset>
                    ...
                    <ul id="photos" class="pic-list unstyled">
                        <?php if(is_array($picarr)): foreach($picarr as $key=>$picitem): ?><li>
                                <img  onerror="this.src='/admin/themes/simplebootx/Public/assets/images/default-thumbnail.png'"
                                      src="<?php echo ($picitem); ?>"  class="ttt"  style="cursor: hand;width: 100px;height: 100px" ondblclick="image_priview('<?php echo ($picitem); ?>');"/>
                                <input required readonly title='双击查看' type="hidden" value="<?php echo ($picitem); ?>"  style="width:310px;" ondblclick="image_priview('<?php echo ($picitem); ?>');" class="input image-url-input picurl">
                                <a href="javascript:;" onclick="removepic(this)">移除</a>
                        ...
                        <li  v-for="photo in goods.photos">
                            ...
                            <input required readonly title='双击查看' type="hidden" value="{{photo}}"  style="width:310px;" ondblclick="image_priview('{{photo}}');" class="input image-url-input picurl">
        ...
        methods:{
            ...
            submit_form:function ()
            {
                ...
                $(".picurl").each(function () {
                    imgarr[index]=$(this).val();
                    index++;
                });
                c.photos=imgarr;
                var id=this.id;
                $.ajax({
                    url: "<?php echo U('edit_content');?>",
                    type: "POST",
                    data: {goods: c,label:label,tj:tj,zzp:zzp,title:title,contents:contents,id:id},
                    dataType: "json",
